...
module CodiceFiscale
  ...
  def self.calculate(params)
    ...
    raise ArgumentError, citizen.errors.full_messages.join(', ') unless citizen.valid?

    citizen.fiscal_code
  end

  def self.extract_from(fiscal_code, part_to_extract)
    unless Codes::PARTS.include?(part_to_extract)
      raise ArgumentError, "Cannot extract #{part_to_extract} from fiscal code"
    end

    fiscal_code = FiscalCode.new(fiscal_code)
    fiscal_code.send("extract_#{part_to_extract}")
  end
  ...
  def self.update_csv_files(are_you_sure = false)
    ...
    CSV.open(config.city_codes_csv_path, 'wb') do |csv|
      CSV.parse(data, col_sep: ';', row_sep: "\r\n").each_with_index do |r, index|
        ...
        raise("Invalid city name: #{c}")           if c.include?(',') || c.blank?
    ...
    CSV.open(config.country_codes_csv_path, 'wb') do |csv|
      CSV.parse(data, col_sep: ';', row_sep: "\r\n").each_with_index do |r, index|
        ...
        raise("Invalid country name: #{c1}")       if c1.include?(',') || c1.blank?
        raise("Invalid country name: #{c2}")       if c2.include?(',') || c2.blank?
